...
* **Knauf Uniflott / Knauf Fugen ($\alpha$-Hemihydrate High-Strength Gypsum):**
  ...
  * Delivers **near-zero volumetric shrinkage** with compressive strength $> 6.0\text{ MPa}$ ($> 870\text{ psi}$) and flexural strength $> 2.5\text{ MPa}$ ($> 360\text{ psi}$).
...
6. [ ] **Fastener Precision:** Drywall screws driven to a uniform depth of $0.8–1.0\text{ mm (1/32 in)}$ without paper tearing. Screw pitch $\le 170\text{ mm (6 in)}$ on ceilings, $\le 250\text{ mm (10 in)}$ on walls.
7. [ ] **Floor Clearance:** Continuous $10\text{ mm (3/8 in)}$ gap maintained between the bottom edge of gypsum boards and the structural floor screed.
8. [ ] **Edge Prep & Taping:** All cut edges beveled to $22.5^\circ$, dusted, primed with Tiefengrund, and reinforced with Knauf Kurt paper tape bedded in Knauf Uniflott setting compound.
