...
* Standard design codes permit mid-span deflection ratios between /250$ and /500$. Across a 6-meter (20 ft) span, permissible structural sag reaches **12 to 24 mm (0.5 to 1.0 in)**.
...
* In double-layer assemblies (W 112 / P 112), stagger both longitudinal and transverse joints between the base layer and face layer by **one full stud spacing (400 or 600 mm / 16 or 24 in)** horizontally and $\ge 400\text{ mm (16 in)}$ vertically.
...
* **Knauf Uniflott / Knauf Fugen ($\alpha$-Hemihydrate High-Strength Gypsum):**
  * Hardens via chemical hydration (crystalline reaction), not moisture evaporation.
  * Delivers **near-zero volumetric shrinkage** with compressive strength $> 6.0\text{ MPa}$ (\text{ psi}$) and flexural strength $> 2.5\text{ MPa}$ (\text{ psi}$).
  * **Requirement:** Structural joint filling and bedding of Kurt tape must be executed exclusively with chemically setting $\alpha$-hemihydrate compounds.
...
1. [ ] **Substructure Steel Gauge:** Verified minimum $\ge 0.60\text{ mm (24 ga)}$ certified steel across all tracks, studs, and furring channels. Zero stripped screws.
...
3. [ ] **Sliding Ceiling Perimeter:** CD 60/27 furring channels remain floating unattached inside UD 28/27 perimeter tracks with $\ge 10\text{ mm (3/8 in)}$ end clearance.
4. [ ] **Joint Stagger Distance:** All transverse board joints staggered by $\ge 400\text{ mm (16 in)}$. Zero cruciform four-corner joints.
5. [ ] **Opening Geometry:** All door and window rough opening corners wrapped with seamless L-shaped boot cut (flag cut) panels offset $\ge 150\text{ mm (6 in)}$ from corners.
6. [ ] **Fastener Precision:** Drywall screws driven to a uniform depth of .8–1.0\text{ mm (1/32 in)}$ without paper tearing. Screw pitch $\le 170\text{ mm (6 in)}$ on ceilings, $\le 250\text{ mm (10 in)}$ on walls.
7. [ ] **Floor Clearance:** Continuous \text{ mm (3/8 in)}$ gap maintained between the bottom edge of gypsum boards and the structural floor screed.
8. [ ] **Edge Prep & Taping:** All cut edges beveled to .5^\circ$, dusted, primed with Tiefengrund, and reinforced with Knauf Kurt paper tape bedded in Knauf Uniflott setting compound.
