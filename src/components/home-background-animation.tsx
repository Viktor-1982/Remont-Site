...
export function HomeBackgroundAnimation() {
    return (
        <div className="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
            ...
            {/* 🎨 Волны краски (плавные кривые) */}
            <div className="absolute inset-0">
                {Array.from({ length: 4 }).map((_, i) => (
                    <div
                        key={`paint-wave-${i}`}
                        className="absolute opacity-5 dark:opacity-8 animate-paint-flow"
                        style={{
                            left: `${i * 25}%`,
                            top: `${20 + i * 20}%`,
                            width: '200px',
            ...
            <div className="absolute inset-0">
                {Array.from({ length: 15 }).map((_, i) => (
                    <div
                        key={`tile-${i}`}
                        className="absolute border border-primary/6 dark:border-primary/10 rounded-sm animate-pulse-slow"
                        style={{
                            left: `${(i * 6.7) % 92}%`,
                            top: `${(i * 9.3) % 88}%`,
                            width: `${30 + (i % 2) * 15}px`,
